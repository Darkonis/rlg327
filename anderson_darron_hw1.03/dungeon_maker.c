...
int main(int argc, char *argv[])
{
 ...
 if (argc > 1) {
  ...
  // d.partialPath=malloc(sizeof(d.partialPath));
  generateDistances(&d);
  int m=0;
  int n=0;
  /*   for(m=0;m<DUNGEON_Y;m++)
  ...
  for(m=0;m<DUNGEON_Y;m++)
    {
      for(n=0;n<DUNGEON_X;n++)
	{	 
	  if(&(d.partialPath[m][n].cost)==NULL||d.map[m][n]==ter_wall_immutable||d.map[m][n]==ter_wall) 
	    {
	      //putchar((char)(d.partialPath[m][n]->cost)+48);
	       putchar(' ');
	    }
	  else if(&(d.partialPath[m][n].cost)!=NULL&&d.partialPath[m][n].cost==0)
	    {
	      putchar('@');
	    } 
	  else
	    {
	      ...
	      putchar((char)((d.partialPath[m][n].cost)%10+48));
	    }
	}
            putchar('\n');
    }
